...
struct freertos_sockaddr xAddress;
	...
	*/
	...
	for( ;; )
	{
		printf("Loop, processing IP events\n");
...
BaseType_t xReturn = pdFALSE;
	...
	printf("FreeRTOS_IPInit: xIPIsNetworkTaskReady, xNetworkEventQueue, xIPTaskHandle \n");
...
//JTTODO			 	configASSERT( ( ( *ipLOCAL_IP_ADDRESS_POINTER ) & xNetworkAddressing.ulNetMask ) == ( xNetworkAddressing.ulGatewayAddress & xNetworkAddressing.ulNetMask ) );
			}
	...
	//		FreeRTOS_debug_printf( ( "FreeRTOS_IPInit: xNetworkBuffersInitialise() failed\n") );
			printf("FreeRTOS_IPInit: xNetworkBuffersInitialise() failed\n");
			/* Clean up. */
	...
	//	FreeRTOS_debug_printf( ( "FreeRTOS_IPInit: Network event queue could not be created\n") );
		printf("FreeRTOS_IPInit: Network event queue could not be created\n");
	}
...
static eFrameProcessingResult_t prvAllowIPPacket( const IPPacket_t * const pxIPPacket,
	NetworkBufferDescriptor_t * const pxNetworkBuffer, BaseType_t lHeaderLength )
{
