 ...
 * determined.
 */
static BaseType_t prvValidSocket( FreeRTOS_Socket_t *pxSocket, BaseType_t ucProtocol, BaseType_t ucIsBound );
 ...
 * (UDP)
 */
int32_t FreeRTOS_recvfrom( Socket_t xSocket, void *pvBuffer, size_t xBufferLength, uint32_t ulFlags, struct freertos_sockaddr *pxSourceAddress, socklen_t *pxSourceAddressLength )
{
...
BaseType_t FreeRTOS_GetLocalAddress( Socket_t xSocket, struct freertos_sockaddr *pxAddress )
{
FreeRTOS_Socket_t *pxSocket = ( FreeRTOS_Socket_t * ) xSocket;
...
/*-----------------------------------------------------------*/
	...
	EventBits_t xEventBits = 0;
		...
		{
			xByteCount = ( BaseType_t ) pxSocket->u.xTcp.rxStream ? uxStreamBufferGetSize ( pxSocket->u.xTcp.rxStream ) : 0;
			...
			if( xByteCount > 0 )
			{
				...
				{
					xByteCount = ( BaseType_t ) uxStreamBufferGet( pxSocket->u.xTcp.rxStream, 0, ( uint8_t * ) pvBuffer, ( size_t ) xBufferLength, (xFlags & FREERTOS_MSG_PEEK) != 0 );
...
/*-----------------------------------------------------------*/
	...
	StreamBuffer_t *pxBuffer = pxSocket->u.xTcp.txStream;
		...
		if( pxBuffer != NULL )
		{
		BaseType_t xSpace = uxStreamBufferGetSpace( pxBuffer );
		BaseType_t xRemain = pxBuffer->LENGTH - pxBuffer->uxHead;
		...
		if( xByteCount > 0 )
		{
			...
			while( xBytesLeft > 0 )
			{
				...
				if( xByteCount > 0 )
				{
					...
					xByteCount = ( BaseType_t ) uxStreamBufferAdd( pxSocket->u.xTcp.txStream, 0, ( const uint8_t * ) pvBuffer, ( size_t ) xByteCount );
			...
			if( xByteCount == 0 )
			{
				...
				{
					xByteCount = ( int32_t ) -pdFREERTOS_ERRNO_ENOTCONN;
				}
				else
				{
					...
					xByteCount = ( int32_t ) -pdFREERTOS_ERRNO_ENOSPC;
				}
			}
		}
...
/*-----------------------------------------------------------*/
	 ...
	 */
	/*_RB_ Should xOffset be a uint32_t, or should it name start with 'x'? */
	int32_t lTCPAddRxdata( FreeRTOS_Socket_t *pxSocket, BaseType_t xOffset, const uint8_t *pcData, uint32_t ulByteCount )
	{
	...
	int32_t xResult;
		...
		/* int32_t uxStreamBufferAdd( pxBuffer, lOffset, pucData, aCount )
