...
const enet_phy_config_t g_enetPhyCfg[ENET_INSTANCE_COUNT] = {{false, 0, false }};
...
uint8_t 	*txBdPtr[ENET_INSTANCE_COUNT],
			*rxBdPtr[ENET_INSTANCE_COUNT];

uint8_t 	*txBuffer[ENET_INSTANCE_COUNT],
			*rxBuffer[ENET_INSTANCE_COUNT];
...
#if FSL_FEATURE_ENET_SUPPORT_PTP	// executes
...
#else
    buffCfgPtr->extRxBuffQue = NULL;	// executes
...
#if !ENET_RECEIVE_ALL_INTERRUPT // doesn't execute
 ...
 */
static struct pbuf * low_level_input(struct netif *netif, enet_mac_packet_buffer_t *packetBuffer)
{
  uint16_t location = 0;
...
err_t ethernetif_init(struct netif *netif)
{
  ...
  /* initialize the hardware */
  	  result = low_level_init(netif);	// ok
...
err_t low_level_init_port()
{
		...
		*/
		...
		g_enetMacCfg[devNumber].macAddr = (uint8_t *) 0x00; // &(netif->hwaddr[0]);
